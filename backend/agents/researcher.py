...
def run_researcher(sub_tasks: list[str], session_id: str, has_documents: bool) -> dict:
    """
    Agent 2 — Researcher.
    ...
    if has_docs:
        ...
        for task in sub_tasks:
            ...
            response = llm.invoke(RAG_PROMPT.format(task=task, context=context))
            answers.append(f"**Sub-task:** {task}\n\n{response.content.strip()}")
